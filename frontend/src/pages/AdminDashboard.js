...
import { useAuth } from "../context/AuthContext"; // Import réel
import { getActivitiesForUser } from "../services/api"; // Nouvelle API pour les activités utilisateur

// 🚀 Composant Button simple (gardé pour la mise en forme)
...
export default function AdminDashboard() {
  ...
  // --- Données spécifiques à l'utilisateur non-admin ---
  const [userActivities, setUserActivities] = useState([]);
  
  // --- Données fictives pour l'admin ---
  ...
  useEffect(() => {
    ...
    if (user?.typeUtilisateur === "administrateur") {
      // --- Logique d'administration simulée (récupérée de l'original) ---
      // Idéalement, getAdminDashboardStats() devrait être appelé ici.
      setTimeout(() => {
        setDashboardStats([
          {
            title: "Total Users",
        ...
        getActivitiesForUser(user.idUtilisateur) // Appelle l'API pour obtenir les activités liées à l'utilisateur
            .then(res => {
                // L'API est censée retourner un tableau d'activités
                setUserActivities(res.data);
                setLoading(false);
            })
            .catch(err => {
                console.error("Erreur de récupération des activités utilisateur:", err);
                setUserActivities([]);
                setLoading(false);
            });
    } else {
        // Utilisateur non connecté ou non défini
        setLoading(false);
    }
  }, [user, authLoading]);

  // --- Suppression simulée pour l'admin ---
